...
p. OSX runs Bash by default, from @man sh@:

<notextile><pre><code data-language="generic">Bash is Copyright (C) 1989-2005 by the Free Software Foundation, Inc.
          ...
...
p. But @~/.bashrc@ is ignored even when launching Terminal.app which according to the Bash manual is considered as *an interactive not a login shell*.

p. *Interactive* means there is a person typing things into the terminal as oppose to the non-interactive command execution such as by a script. *Login shell* means you have to authenticate with a username and password to start the shell as oppose to simply launching Terminal.app in an already authenticated on environment.
