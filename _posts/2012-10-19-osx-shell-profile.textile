...
p. Setting up shell profile on OSX can get messy, especially when one has to do it only once or twice a year. Typically I start by poking at the obvious @~/.profile@ with no apparent success, discover @~/.bashrc@, @~/.bash_profile@, read some stackoverflow posts and finally the shell man pages. 
...
p. From the man page and poking around, here is a run down of files executed when launching *Terminal.app*.
# Always run system wide @/etc/profile@ and @/etc/bashrc@
# Only first that exists @~/.bash_profile@, @~/.bash_login@, @~/.profile@ in that order

p. If @~/.bash_profile@ exists and contains a single blank line then @~/.profile@ is completely ignored. And this gets me every time!